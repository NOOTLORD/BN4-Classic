...
//=============================================================================
class AM67SecondaryFire extends BallisticInstantFire;
...
simulated state CombatLaser
{
	...
	{
		if ((AM67Pistol(Weapon).LaserAmmo < AM67Pistol(Weapon).default.LaserAmmo && !bLaserFiring) || level.TimeSeconds - StopFireTime < 0.75 || AM67Pistol(Weapon).LaserAmmo <= 0 || !super.AllowFire())
		{
			if (bLaserFiring)
				StopFiring();
			return false;
	...
	simulated event ModeDoFire()
	{
		...
		LaserAmmoPerFire = default.LaserAmmoPerFire * (1 + 4*FMax(0, AM67Pistol(Weapon).LaserAmmo - 0.5));
		super.ModeDoFire();
	}

	function ApplyDamage(Actor Victim, int Damage, Pawn Instigator, vector HitLocation, vector MomentumDir, class<DamageType> DamageType)
	{
		super.ApplyDamage (Victim, Damage * (1 + 4*FMax(0, AM67Pistol(Weapon).LaserAmmo - 0.4)), Instigator, HitLocation, MomentumDir, DamageType);
		
		if (Victim.bProjTarget)
		{
			BW.TargetedHurtRadius(5 * (1 + 4*FMax(0, AM67Pistol(Weapon).LaserAmmo - 0.4)), 20, class'DTSARLaser', 0, HitLocation, Pawn(Victim));
		}
	}
	...
	simulated function bool ImpactEffect(vector HitLocation, vector HitNormal, Material HitMat, Actor Other, optional vector WaterHitLoc)
	{
		if (Other == None || Other.bWorldGeometry)
			BW.TargetedHurtRadius(5 * (1 + 4*FMax(0, AM67Pistol(Weapon).LaserAmmo - 0.4)), 20, class'DTSARLaser', 50, HitLocation);
...
simulated state LaserSight
{
	...
	event ModeDoFire()
	{
		if (Weapon.Role == ROLE_Authority)
			AM67Pistol(Weapon).ServerSwitchlaser(!AM67Pistol(Weapon).bLaserOn);
	}
}
