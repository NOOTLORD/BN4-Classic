...
//  New deathmessage class to decouple the killfeed from the chat and move the killfeed to the top right
// ======================================================================================================
class BN4Deathmessage extends LocalMessage;
	...
	)
{
		...
		|| (P.PlayerReplicationInfo.bOnlySpectator && (Pawn(P.ViewTarget) != None) && (Pawn(P.ViewTarget).PlayerReplicationInfo == RelatedPRI_1)) )
	{
        ...
        if ( P.Role == ROLE_Authority )
        {
			...
			if ( UnrealPlayer(P).MultiKillLevel > 0 )
				P.ReceiveLocalizedMessage( class'BN4MultiKillMessage', UnrealPlayer(P).MultiKillLevel );
		}
        else
        {
				...
				&& ((RelatedPRI_2.Team == None) || (RelatedPRI_1.Team != RelatedPRI_2.Team)) )
			{
				if ( (P.Level.TimeSeconds - UnrealPlayer(P).LastKillTime < 4) && (Switch != 1) )
				{
					...
					P.ReceiveLocalizedMessage( class'BN4MultiKillMessage', xPlayer(P).MultiKillLevel );
				} 
				else
	...
	{
		P.ReceiveLocalizedMessage( class'BN4VictimMessage', 0, RelatedPRI_1 );
		if ( !Default.bNoConsoleDeathMessages )
			Super.ClientReceive(P, Switch, RelatedPRI_1, RelatedPRI_2, OptionalObject);
	} 
